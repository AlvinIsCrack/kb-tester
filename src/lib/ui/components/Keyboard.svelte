<script lang="ts">
	...
	import { Key } from '$lib/types/keyboard';
	...
	const { keyboard, keyboardRow, navKeys, arrowKeys, numpad, specialRow } = tv({
		slots: {
			...
			specialRow: `flex ${gap} items-center flex-row w-full absolute -top-4 -translate-y-full justify-start`
		},
		compoundSlots: [
			{
				slots: ['keyboard', 'keyboardRow'],
				class: `flex ${gap} justify-start items-center`
			}
		],
		variants: {
			size: {
...
<div class="mt-8 flex flex-row justify-center will-change-transform">
	...
	<div class="{keyboard({})} {!Page.numpadEnabled ? 'ml-6' : ''}">
		...
		<div
			class="{Page.numpadEnabled
				? 'peer'
				: ''} absolute top-0 right-0 z-10 -m-4 origin-top scale-80 transition-all duration-200"
		>
			<Button
